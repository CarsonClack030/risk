...
- `src/`: React 前端工作台；主控制器、页面区块、业务弹窗和纯数据转换分别维护
- `src-tauri/`: Tauri 桌面壳与 sidecar 启动逻辑
- `backend/`: Python HTTP 层、应用服务、导入导出、计算服务、嵌入数据库与 sidecar 构建脚本
...
代码还会对本地服务做几层保护：桌面壳每次启动生成一次性 API 令牌和退出令牌，管理员
登录使用带盐 PBKDF2 哈希并有失败次数限制；参数、浓度、场地选项和工作区序号会在后端
再次校验。Excel/XLS/CSV/TXT 导入限制文件体积、工作表行列和单元格数量，损坏文件会以
可读错误返回，不会把异常堆栈直接展示给用户。运行数据库迁移前会保留最多 3 份备份。

`npm run lint` 会同时检查前端和 Python；Ruff、ESLint 都属于开发依赖，不会进入最终安装包。
...
注意：Gitee 私有仓库不会向未登录的桌面客户端公开 Release 信息，而且不能把私人令牌写进
软件安装包。面向其他用户分发更新前，需要把仓库或专用下载仓库设为公开。
...
导入上限为 10000 行、256 列和 1000000 个单元格，XLSX 压缩包解压后最多 64 MB；这些
限制用于避免误选大型文件或异常压缩包导致本地后端占满内存。工作区最多保存 10000 条
记录，同一个污染物仍然可以重复加入，但污染物库记录被工作区引用时不能直接删除或修改。
...
其中 `nsis` 目录里会有 Windows 安装包 `.exe`。GitHub 仓库也可以直接使用
`.github/workflows/windows-build.yml`，在 `windows-latest` 上自动产出可下载的构建产物。
