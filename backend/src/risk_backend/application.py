...
class RiskBackend:
    ...
    def update_concentrations(
        ...
        items = [
            PollutantConcentration(
                ...
                surface_concentration=self._parse_finite_decimal(
                    item.get("surface_concentration", 0), "地表浓度"
                ),
                lower_soil_concentration=self._parse_finite_decimal(
                    item.get("lower_soil_concentration", 0), "下层土壤浓度"
                ),
                groundwater_concentration=self._parse_finite_decimal(
                    item.get("groundwater_concentration", 0), "地下水浓度"
                ),
                groundwater_protection_concentration=self._parse_finite_decimal(
                    item.get("groundwater_protection_concentration", 0),
                    "地下水保护浓度",
                ),
            )
    ...
    def calculate(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        pathways = {key: bool(pathway_payload.get(key)) for key in PATHWAY_KEYS}
        ...
        for workspace_number, table_values in results.items():
            for table_name, values in table_values.items():
                self.result_repository.update_table(
                    table_name, workspace_number, values
                )
    ...
    def _build_pollutant(
        ...
        values = {
            field: self._parse_finite_decimal(payload.get(field, default), label)
