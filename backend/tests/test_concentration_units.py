...
from risk_backend.workspace_import import (
...
class ConcentrationUnitTests(unittest.TestCase):
    ...
    def test_import_recognizes_headers_with_units(self) -> None:
        ...
        _, column_map = WorkspaceImporter.parse_columns(rows)

        self.assertTrue(
            all(field in column_map for field in EXCEL_CONCENTRATION_FIELDS)
        )

    def test_import_keeps_support_for_legacy_headers(self) -> None:
        rows = [
            ["污染物名称", "地表浓度", "下层土壤浓度", "地下水浓度", "地下水保护浓度"]
        ]
